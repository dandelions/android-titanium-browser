...
当前源码基于 Chromium `152.0.7977.84`，版本号由 `vanadium/args.gn` 自动读取。
...
```text
chromium/src/out/release/152.0.7977.84-arm64-v8a.apk
```
...
两边都应显示 `152.0.7977.84`。`hotfix_existing_src.sh` 负责把新的下游补丁应用到已有源码，但不会把旧版 Chromium 升级成 `152.0.7977.84`。版本不一致时必须执行完整构建，让 `build.sh` 重新获取目标 Chromium 标签并应用 Vanadium 补丁。
...
新版 `build.sh` 和 `hotfix_existing_src.sh` 会在子模块提交、Chromium 版本或 DrumBrake 子项目补丁不匹配时提前停止，避免生成版本号不符的 APK。
...
```text
android_default_version_name = "152.0.7977.84"
```
