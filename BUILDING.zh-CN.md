...
当前源码基于 Chromium `152.0.7977.42`，版本号由 `vanadium/args.gn` 自动读取。
...
```text
chromium/src/out/release/152.0.7977.42-arm64-v8a.apk
```
...
两边都应显示 `152.0.7977.42`。`hotfix_existing_src.sh` 负责把新的下游补丁应用到已有源码，但不会把 Chromium `151.0.7922.137` 升级成 `152.0.7977.42`。版本不一致时必须执行完整构建，让 `build.sh` 重新获取目标 Chromium 标签并应用 Vanadium 补丁。

如果现有源码是 `151.0.7922.137`，不要继续 Localbuild。v152.0.7977.42 仍依赖 Vanadium V8 子项目补丁的 DrumBrake；旧源码缺少该补丁时会在 `v8/BUILD.gn` 报 `DrumBrake is only available`。清理子模块后直接执行一次完整构建：
...
新版 `build.sh` 和 `hotfix_existing_src.sh` 会在子模块提交、Chromium 版本或 DrumBrake 子项目补丁不匹配时提前停止，避免生成文件名为 `.42`、实际源码却仍是 `.137` 的 APK。
...
```text
android_default_version_name = "152.0.7977.42"
```
