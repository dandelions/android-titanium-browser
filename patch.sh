...
RESOLVED_VERSION="${VERSION:-$(grep -m1 -o '[0-9]\+\(\.[0-9]\+\)\{3\}' "$SCRIPT_DIR/vanadium/args.gn" 2>/dev/null || true)}"
...
python3 - "chrome/android/java/src/org/chromium/chrome/browser/tabbed_mode/TabbedAppMenuPropertiesDelegate.java" "chrome/android/java/res/values/ids.xml" "chrome/android/java/src/org/chromium/chrome/browser/app/ChromeActivity.java" <<'PYCODE'
from pathlib import Path
import re
import sys
...
grep -q 'int g_last_android_extension_action_tab_id = -1;' "$MENU_DELEGATE_CC" || sed -i '/constexpr gfx::Size kActionIconSize = /a\
...
' "$MENU_DELEGATE_CC"
grep -q 'static base::NoDestructor<base::WeakPtr<content::WebContents>> storage;' "$MENU_DELEGATE_CC" || sed -i '/int g_last_android_extension_action_tab_id = -1;/a\
base::WeakPtr<content::WebContents>&\
GetLastAndroidExtensionActionWebContentsStorage() {\
  static base::NoDestructor<base::WeakPtr<content::WebContents>> storage;\
  return *storage;\
