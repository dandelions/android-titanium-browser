...
USER_SCRIPT_SET=extensions/renderer/user_script_set.cc
...
ARCORE_MANIFEST=third_party/arcore-android-sdk-client/AndroidManifest_basesplit.xml
MANIFEST_FEATURES=chrome/common/extensions/api/_manifest_features.json
...
# Restore declared extension run_at timing. Older Helium patches delayed document_start/document_end scripts
# and skipped streaming on Android main frames, which prevents script-at extensions from running.
python3 - "$USER_SCRIPT_SET" <<'PYCODE'
from pathlib import Path
import sys
...
custom_start = text.find("  // Delay extension main-frame work until the page is idle.")
if custom_start >= 0:
    custom_end = text.find(
        "      !script->js_scripts().empty() && script_run_location == run_location;",
        custom_start,
    )
    if custom_end < 0:
        raise SystemExit(f"custom script timing block end not found in {path}")
    custom_end += len(
        "      !script->js_scripts().empty() && script_run_location == run_location;"
    )
    text = text[:custom_start] + """  bool inject_css = !script->css_scripts().empty() &&
                    run_location == mojom::RunLocation::kDocumentStart;
  bool inject_js =
      !script->js_scripts().empty() && script->run_location() == run_location;""" + text[custom_end:]
path.write_text(text)
PYCODE
...
RESOLVED_VERSION="${TARGET_VERSION:-${CHROMIUM_VERSION:-$(grep -m1 -o '[0-9]\+\(\.[0-9]\+\)\{3\}' "$VERSION_ARGS" 2>/dev/null || true)}}"
if [ -n "$RESOLVED_VERSION" ] && ! version_lt "$RESOLVED_VERSION" "153.0.0.0"; then
    POPUP_CONTENTS_CREATE_CALL="ExtensionActionPopupContents.create(nativeHostPtr, false)"
    POPUP_CONSTRUCTOR_EXTRA_ARGS=",\
                        false"
    TRIGGER_POPUP_FALLBACK="toolbar_android_->TriggerPopup(action_id_, std::move(host), show_action);"
else
    POPUP_CONTENTS_CREATE_CALL="ExtensionActionPopupContents.create(nativeHostPtr)"
    POPUP_CONSTRUCTOR_EXTRA_ARGS=""
    TRIGGER_POPUP_FALLBACK="toolbar_android_->TriggerPopup(action_id_, std::move(host));"
fi
...
\
    private void showActionPopup(String actionId, long nativeHostPtr) {\
        ExtensionActionPopupContents contents = '"$POPUP_CONTENTS_CREATE_CALL"';\
        ...
        mActivePopup =\
                new ExtensionActionPopup(\
                        ...
                        mContextMenuPopulatorFactory,\
                        mSelectionDropdownMenuDelegate,\
                        mTabModelSelector'"$POPUP_CONSTRUCTOR_EXTRA_ARGS"');\
